defmodule KinoExtras.DynamicRemoteExecutionCell do
  @moduledoc false

  use Kino.JS, assets_path: "lib/assets/dynamic_remote_execution_cell/build"
  use Kino.JS.Live
  use Kino.SmartCell, name: "Dynamic remote execution"
  ...
  defp intellisense_node(fields, node_options, cookie_options) do
    ...
    if is_binary(node) and node =~ "@|dynamic:" and is_binary(cookie) and cookie != "" do
      {resolve_node(node), String.to_atom(cookie)}
    end
  end

  def resolve_node("dynamic:" <> node) do
    [app_name, discovery_service] = String.split(node, ":")

    {:ok, {:hostent, _fqdn, [], :inet, _value, addresses}} =
      :inet_res.getbyname(String.to_charlist(discovery_service), :a)

    {a, b, c, d} = hd(addresses)
    :"#{app_name}@#{a}.#{b}.#{c}.#{d}"
  end

  def resolve_node(node), do: String.to_atom(node)
  ...
  defp build_node(%{"node_secret" => secret}) do
    quote do
      ...
      |> KinoExtras.DynamicRemoteExecutionCell.resolve_node()
    end
  end

  defp build_node(%{"node_text" => node}) do
    quote do
      KinoExtras.DynamicRemoteExecutionCell.resolve_node(unquote(node))
    end
  end

  defp build_node(%{"node_variable" => variable}) do
    quote do
      unquote({String.to_atom(variable), [], nil})
      |> KinoExtras.DynamicRemoteExecutionCell.resolve_node()
    end
  end
